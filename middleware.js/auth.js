...
async function logAudit(userId, action, resource, resourceId, req, status, metadata = {}) {
    try {
        const ip = req.ip || req.headers['x-forwarded-for'] || 'unknown';
        await query(
            `INSERT INTO audit_logs (user_id, action, resource, resource_id, ip, status, metadata)
             VALUES ($1, $2, $3, $4, $5, $6, $7)`,
            [userId, action, resource, String(resourceId), ip, status, JSON.stringify(metadata)]
        );
    } catch (err) {
